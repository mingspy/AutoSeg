                       ...
                       ITokenizer & tokenizer, int choice)
{
    ...
    cout <<"data\t elapsed\t speed\t words \t segmented \t corrected \t precision \t recall \t F2"<<endl
        <<test_size<<"bytes\t"<<elapsed<<"ms\t"<<(test_size/1024.0/1024.0/elapsed)<<"m/s\t"<<refer_words
        <<"\t"<<total_segmented<<"\t"<<total_correct<<"\t"<<precision<<"\t"<<recall<<"\t"<<f2<<endl;
}
